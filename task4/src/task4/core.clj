...
(defn notify-msg
  ...
  (let [state1 (assoc state
                 :buffer
                 (let [bill-cnt (get (state :bill) ware),
                       buffer-cnt (get (state :buffer) ware)]
                   (if (> bill-cnt buffer-cnt)
                     (let [ware-cnt-to-get (min (- bill-cnt buffer-cnt) @storage-atom)]
                       (try
                         (swap! storage-atom #(- % ware-cnt-to-get))
                         (assoc (state :buffer) ware (+ buffer-cnt ware-cnt-to-get))
                         (catch IllegalStateException e (state :buffer))))
                     (state :buffer))))]
